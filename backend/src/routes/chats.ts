...
chatRoutes.post("/:id/messages/stream", rateLimit(20, 60_000), async (c) => {
  ...
  return stream(c, async (output) => {
    ...
    try {
      ...
      const modelIdentity = `You are currently responding as ${providerConfig.model} (${providerConfig.provider}).`;
      const systemMessages = [
        {
          role: "system" as const,
          content: `${modelIdentity}\nCurrent UTC date and time: ${currentDate}\n\n${userSettings?.system_prompt || DEFAULT_SYSTEM_PROMPT}`,
        },
      ];
      ...
      const assistantMsgIds = storedMessages.filter((m) => m.role === "assistant").map((m) => m.id);
      const receiptMap = await getReceiptsForMessages(assistantMsgIds);
      const annotatedMessages = storedMessages.map((m) => {
        if (m.role !== "assistant") return m;
        const info = receiptMap.get(m.id);
        if (!info || (info.model === providerConfig.model && info.provider === providerConfig.provider)) return m;
        return { ...m, content: `[This response was generated by ${info.model} (${info.provider})]\n${m.content}` };
      });

      let context = selectContext(systemMessages, annotatedMessages, chat.summary, maxInputTokens);
      ...
      if (unsummarized.length > 0) {
        ...
        try {
          ...
          await db
            ...
            .where("id", "=", chat.id)
            .execute();
          context = selectContext(systemMessages, annotatedMessages, summary, maxInputTokens);
          contextStats = context.stats;
        } catch {
          context = selectContext(systemMessages, annotatedMessages, chat.summary, maxInputTokens);
          contextStats = context.stats;
        }
      }
